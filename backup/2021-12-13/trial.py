...
#    cue_mat,fs = medussa.read_file(fillStimNames('ba',spaCond,tarDir,'T',intCond)) 
    cue_mat,fs,_enc = pysndfile.sndio.read(fillStimNames('ba',spaCond,tarDir,'T',intCond))
...
#        stim_mat,fs = medussa.read_file(fillStimNames(stream[i],spaCond,tarDir,codes[i],intCond))
        stim_mat,fs,_enc = pysndfile.sndio.read(fillStimNames(stream[i],spaCond,tarDir,codes[i],intCond))
    ...
    #zero_paddings = np.zeros((full_stream.shape[0],6)) # set 14-20 all 0?
    #trig_chan = np.concatenate((zero_paddings,trig_chan),axis=1)
    trig_chan = np.tile(trig_chan,(1,34)) # TODO: I've been playing the trigger through all of the left 18 channels, but still not seeing any signal
